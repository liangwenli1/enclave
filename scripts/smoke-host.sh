#!/usr/bin/env bash
# 真机冒烟：用清单里真实的内核走一遍 准入 → 启动 → 调试端口握手 → 停止。
...
echo "== 准入（会下载内核，约 140–190 MB）"
call -X POST -d '{"allowPreviewChannel":true}' "$base/v1/kernel/admit" >/dev/null
for i in $(seq 1 450); do
  view="$(call "$base/v1/kernel")"
  state="$(echo "$view" | field 'd["status"]["state"]')"
  [ $((i % 10)) -eq 0 ] && echo "   $state $(echo "$view" | field 'd["status"].get("bytesReceived")')"
  case "$state" in
    admitted) break ;;
    error|hash_mismatch) echo "准入失败：$(echo "$view" | field 'd["status"].get("error")')"; exit 1 ;;
  esac
  sleep 2
done
[ "$state" = "admitted" ] || { echo "准入超时，停在 $state"; exit 1; }
exe="$(echo "$view" | field 'd["status"]["executable"]')"
echo "   可执行文件：$exe"
...
spec='{"envId":"env_smoke","allowNoSandbox":'"$no_sandbox"',"allowPreviewChannel":true,"extraFlags":[],"profile":{"seed":"424242","platform":"windows","platformVersion":"19.0.0","brand":"Chrome","brandVersion":"148.0.7778.215","hardwareConcurrency":8,"locale":"en-US","languages":["en-US","en"],"timezone":"America/Los_Angeles","screen":{"width":1920,"height":1080},"webrtc":{"mode":"replace"}}}'
